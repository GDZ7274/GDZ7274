    ...
    while True:
        ...
        if len(list_bboxs) > 0:
            ...
            for item_bbox in list_bboxs:
                ...
                x_center, y_center = objtracker.get_bbox_coordinate(item_bbox)
                center_x, center_y = (x1+x2)//2, (y1+y2)//2 # 计算目标中心位置
                cv2.putText(output_image_frame, f"ID:{track_id} ({center_x},{center_y})", 
                    (x1, y1-30), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 2)
                
                current_time = time.time()
                elapsed_time = current_time - start_time
                #if elapsed_time - timer > 1:
                   # print('track_id:', track_id, ', coordinates:', center_x, center_y)
                   # timer = elapsed_time
